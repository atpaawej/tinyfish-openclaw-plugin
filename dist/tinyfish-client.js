...
const safeRetryAfter = (value) => { const n = value ? Number(value) : NaN; return Number.isFinite(n) && n >= 0 ? Math.min(n, 3600) : undefined; };
    ...
    async request(url, init, signal) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                const kind = response.status === 401 || response.status === 403 ? "auth" : response.status === 429 ? "rate-limit" : response.status >= 500 ? "transient" : "http";
            ...
            try {
                ...
                throw new TinyFishError("invalid-response", "TinyFish returned invalid JSON.");
            }
        }
        catch (error) {
            ...
            if (controller.signal.aborted)
                throw new TinyFishError("timeout", "TinyFish request timed out.");
